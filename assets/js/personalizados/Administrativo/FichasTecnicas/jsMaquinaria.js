var dtTblAnexosMaquinaria = $('#TblAnexosMaquinaria').DataTable({
	language: {
	...
	createdRow: function(row, data, dataIndex){
		$(row).on("click", ".eliminarAnexoEquipo", function(e){
			...
			alertify.confirm('Eliminar Anexo', '¿Está seguro de eliminar el registro?', function(){
				mRastreo = 'Elimina anexo Maquinaria Id '+ id;
...
$("[id=CrearAnexosMaquinaria]").on("click", function(e){
	e.preventDefault();
	var row = {
	...
	dtTblAnexosMaquinaria.row.add(row).draw();
	$(this).attr('disabled', true);
});

$("[id=TblAnexosMaquinaria]").on("click", ".LimpiarAN", function(e){
	e.preventDefault();
	dtTblAnexosMaquinaria.row( $(this).closest('tr') ).remove().draw();
	$("#CrearAnexosMaquinaria").attr('disabled', false);
});


var dtTblOperacionesTiempoMaquinaria = $('#TblOperacionesTiempoMaquinaria').DataTable({
	...
	createdRow: function(row, data, dataIndex){
		...
		$(row).on("click", ".btnActualizarOT", function(e){
			...
			var tipo = $(this).closest("[id=TblOperacionesTiempoMaquinaria]").attr("data-tipo");
		...
		$(row).on("click", ".btnEliminarItemOT", function(e){
			...
			alertify.confirm('Eliminar Oprecion Tiempo', '¿Está seguro de eliminar el registro?', function(){
				...
				eliminarOperacion(id, mRastreo, 'Registro eliminado exitosamente','Maquinaria');
			}, function(){});
		});
...
$("[id=CrearOperacionTiempo]").on("click", function(e){
	...
	dtTblOperacionesTiempoMaquinaria.row.add(row).draw();
	$("#TblOperacionesTiempoMaquinaria tbody tr").first().find("td").not(":eq(0),:eq(3),:eq(4)").attr('contenteditable', true).on('keydown paste', function(e){
		if($(this).index() == 1){
			var tamano = 100;
	...
	$("#TblOperacionesTiempoMaquinaria tbody tr").first().find("td").eq(1).focus();
	$(this).attr('disabled', true);
	$('.date').datetimepicker({
		format: 'YYYY-MM-DD',
		locale: 'es'
...
$("[id=TblOperacionesTiempoMaquinaria]").on("click", ".cancelarOPT", function(e){
	e.preventDefault();
	cargarDatosTablas('Operacion',$ID,'ItemEquipoId','OT');
...
var dtTblOperacionesConsumoMaquinaria = $('#TblOperacionesConsumoMaquinaria').DataTable({
	language: pa.language,
	processing: true,
	pageLength: 5,
	bLengthChange: false,
	...
	createdRow: function(row, data, dataIndex){
		...
		$(row).on("click", ".btnActualizarOT", function(e){
			...
			var tipo = $(this).closest("[id=TblOperacionesConsumoMaquinaria]").attr("data-tipo");
			var id = $(this).closest('tr').find('td:eq(0) .btnActualizarOT').val();
			
			mRastreo = 'Actualiza Ficha tecnica Oreción ConsumoMaquinaria  '+ $data.operacion;
			actualizarDatos($data,tipo,'Operacion',mRastreo,$ID,'ItemEquipoId',id,'OperacionId',);
...
$("[id=CrearOperacionConsumoMaquinaria]").on("click", function(e){
	e.preventDefault();
	var undMedida = '<select class="form-control form-control-sm undMedidaTiempo">';
	undMedida += '<option value="001">Dias</option>';
	undMedida += '<option value="002">Semanas</option>';
	...
	dtTblOperacionesConsumoMaquinaria.row.add(row).draw();
	$("#TblOperacionesConsumoMaquinaria tbody tr").first().find("td").not(":eq(0)").attr('contenteditable', true).on('keydown paste', function(e){
	...
	$("#TblOperacionesConsumoMaquinaria tbody tr").first().find("td").eq(1).focus();
	$(this).attr('disabled', true);
	$('.date').datetimepicker({
		format: 'YYYY-MM-DD',
		locale: 'es'
...
$("[id=TblOperacionesConsumoMaquinaria]").on("click", ".cancelarOPT", function(e){
	e.preventDefault();
	dtTblOperacionesConsumoMaquinaria.row( $(this).closest('tr') ).remove().draw();
