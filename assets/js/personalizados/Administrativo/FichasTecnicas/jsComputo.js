	var dtTblAnexosComputo = $('#TblAnexosComputo').DataTable({
		language: {
		...
		createdRow: function(row, data, dataIndex){
			$(row).on("click", ".eliminarAnexoEquipo", function(e){
				...
				alertify.confirm('Eliminar Anexo', '¿Está seguro de eliminar el registro?', function(){
					mRastreo = 'Elimina anexo Computo Id '+ id;
	...
	$("[id=CrearAnexosComputo]").on("click", function(e){
		e.preventDefault();
		var row = {
		...
		dtTblAnexosComputo.row.add(row).draw();
		$(this).attr('disabled', true);
	});

	$("[id=TblAnexosComputo]").on("click", ".LimpiarAN", function(e){
		e.preventDefault();
		dtTblAnexosComputo.row( $(this).closest('tr') ).remove().draw();
		$("#CrearAnexosComputo").attr('disabled', false);
	});



	var dtTblOperacionesTiempoComputo= $('#TblOperacionesTiempoComputo').DataTable({
	...
	createdRow: function(row, data, dataIndex){
		...
		$(row).on("click", ".btnActualizarOT", function(e){
			...
			var tipo = $(this).closest("[id=TblOperacionesTiempoComputo]").attr("data-tipo");
		...
		$(row).on("click", ".btnEliminarItemOT", function(e){
			...
			alertify.confirm('Eliminar Oprecion Tiempo', '¿Está seguro de eliminar el registro?', function(){
				...
				eliminarOperacion(id, mRastreo, 'Registro eliminado exitosamente','Computo');
			}, function(){});
		});
	...
	$("[id=CrearOperacionTiempo]").on("click", function(e){
		...
		dtTblOperacionesTiempoComputo.row.add(row).draw();
		$("#TblOperacionesTiempoComputo tbody tr").first().find("td").not(":eq(0),:eq(3),:eq(4)").attr('contenteditable', true).on('keydown paste', function(e){
			if($(this).index() == 1){
				var tamano = 100;
		...
		$("#TblOperacionesTiempoComputo tbody tr").first().find("td").eq(1).focus();
		$(this).attr('disabled', true);
		$('.date').datetimepicker({
			format: 'YYYY-MM-DD',
			locale: 'es'
	...
	$("[id=TblOperacionesTiempoComputo]").on("click", ".cancelarOPT", function(e){
		e.preventDefault();
		cargarDatosTablas('Operacion',$ID,'ItemEquipoId','OT');
	...
	var dtTblOperacionesConsumoComputo = $('#TblOperacionesConsumoComputo').DataTable({
		language: pa.language,
		processing: true,
		pageLength: 5,
		bLengthChange: false,
	...
	createdRow: function(row, data, dataIndex){
		...
		$(row).on("click", ".btnActualizarOT", function(e){
			...
			var tipo = $(this).closest("[id=TblOperacionesConsumoComputo]").attr("data-tipo");
			var id = $(this).closest('tr').find('td:eq(0) .btnActualizarOT').val();
			
			mRastreo = 'Actualiza Ficha tecnica Oreción ConsumoComputo  '+ $data.operacion;
			actualizarDatos($data,tipo,'Operacion',mRastreo,$ID,'ItemEquipoId',id,'OperacionId',);
	...
	$("[id=CrearOperacionConsumoComputo]").on("click", function(e){
		e.preventDefault();
		var undMedida = '<select class="form-control form-control-sm undMedidaTiempo">';
		undMedida += '<option value="001">Dias</option>';
		undMedida += '<option value="002">Semanas</option>';
		...
		dtTblOperacionesConsumoComputo.row.add(row).draw();
		$("#TblOperacionesConsumoComputo tbody tr").first().find("td").not(":eq(0)").attr('contenteditable', true).on('keydown paste', function(e){
		...
		$("#TblOperacionesConsumoComputo tbody tr").first().find("td").eq(1).focus();
		$(this).attr('disabled', true);
		$('.date').datetimepicker({
			format: 'YYYY-MM-DD',
			locale: 'es'
	...
	$("[id=TblOperacionesConsumoComputo]").on("click", ".cancelarOPT", function(e){
		e.preventDefault();
		dtTblOperacionesConsumoComputo.row( $(this).closest('tr') ).remove().draw();
