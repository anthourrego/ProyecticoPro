var dtOperacionTM = $('#tblOperacionTM').DataTable({
	language,
	processing: true,
	pageLength: 10,
	bLengthChange: false,
	...
	createdRow: function(row, data, dataIndex){
		$(row).on("click", ".cancelar", function(e){
			e.preventDefault();
			dtOperacionTM.row( $(this).closest('tr') ).remove().draw();
...
var dtOperacionCM = $('#tblOperacionCM').DataTable({
	language,
	processing: true,
	pageLength: 10,
	bLengthChange: false,
	...
	createdRow: function(row, data, dataIndex){
		$(row).on("click", ".cancelar", function(e){
			e.preventDefault();
			dtOperacionCM.row( $(this).closest('tr') ).remove().draw();
			$(".btnOP").attr('disabled', false);
			GTBL = '';
		});
	}
...
var dtAnexoM = $('#tblAnexoM').DataTable({
	language,
	processing: true,
	pageLength: 10,
	bLengthChange: false,
	...
	createdRow: function(row, data, dataIndex){
		$(row).on("click", ".cancelar", function(e){
			e.preventDefault();
			dtAnexoM.row( $(this).closest('tr') ).remove().draw();
