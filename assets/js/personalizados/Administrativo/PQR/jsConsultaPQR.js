...
var config = {
	data:{
		tblID : "#tblConsultarPQR",
		select: [
			'DISTINCT hpqr.PQRId', 'CAST(hpqr.Fecha AS DATE) AS Fecha', 'hpqr.Pedido', 'hpqr.Fabricado', 'hpqr.Factura',

			"CASE WHEN hpqr.Producto IS NULL THEN STUFF(( SELECT \
						CONCAT(CAST(ROW_NUMBER() OVER (ORDER BY pq2.ProductoPQRId DESC) AS VARCHAR),' : ',CASE WHEN pq2.ProductoId IS NOT NULL AND pq2.Producto IS NOT NULL THEN pq2.Producto WHEN pq2.ProductoId = '' OR pq2.ProductoId IS NULL THEN pq2.Producto ELSE hp.nombre END \
			...
			'hpqr.TerceroId', 't.nombre AS Cliente',
			'c.nombre AS CiudadCliente', 'hpqr.Descripcion',
			...
			'tpqr.Nombre AS TipoPQR', "hpqr.Devoluciones AS Devoluciones",
			'hpqr.FechaCierre AS FechaSolucion', "hpqr.Costos AS CostosPQR",
			...
			"RESPUESTACLIENTE = STUFF((SELECT  cast(n.Fecha AS varchar) + ': ' + n.Detalle  +  '\r' FROM DetallePQR n LEFT JOIN EstadoPQR e ON e.EstadoId = n.EstadoReporte WHERE n.PQRId = hpqr.PQRId and n.Origen = 'c' FOR XML PATH('p')),1, 0, '')",
			'hpqr.FechaCierre AS FechaCierre', 'epqr.Nombre AS Estado',
		...
		column_filter: [
			...
			, "(CASE WHEN hpqr.Producto IS NULL THEN \
				STUFF(( SELECT \
						CAST(ROW_NUMBER() OVER (ORDER BY pq2.ProductoPQRId DESC) AS VARCHAR) + ' : '+ CASE WHEN pq2.ProductoId IS NOT NULL AND pq2.Producto IS NOT NULL THEN pq2.Producto ELSE CASE WHEN pq2.ProductoId = '' OR pq2.ProductoId IS NULL THEN pq2.Producto ELSE hp.nombre END END\
							+ '\r' FROM ProductoPQR pq2 \
						LEFT JOIN Producto P2 ON pq2.ProductoId = P2.productoid \
						LEFT JOIN HeadProd hp ON P2.headprodid = hp.headprodid \
						WHERE pq2.PQRId = hpqr.PQRId FOR XML PATH('p') \
					), 1, 0, '')\
			ELSE hpqr.Producto END)",
			"(CASE WHEN hpqr.Material IS NULL THEN \
				STUFF(( SELECT \
						CAST(ROW_NUMBER() OVER (ORDER BY pq2.ProductoPQRId DESC) AS VARCHAR) + ' : '+ CASE WHEN pq2.MaterialId = '' OR pq2.MaterialId IS NULL THEN pq2.Material ELSE hpM.nombre END \
						+ '\r' FROM ProductoPQR pq2 \
			...
			ELSE CAST(hpqr.Producidos AS VARCHAR) END)",
			'hpqr.TerceroId'
			,'t.nombre'
			,'c.nombre'
			,'hpqr.Descripcion'
			,"STUFF((SELECT  CAST(n.Fecha AS varchar)+ ': ' + n.Detalle + '\r'  FROM DetallePQR n LEFT JOIN EstadoPQR e ON e.EstadoId = n.EstadoReporte WHERE n.PQRId = hpqr.PQRId and e.Cierre <> 'TRUE' FOR XML PATH('p')),1, 0, '')"
			,"STUFF((SELECT  CAST(n.Fecha AS varchar) + ': ' + n.Detalle  +  '\r' FROM DetallePQR n LEFT JOIN EstadoPQR e ON e.EstadoId = n.EstadoReporte WHERE n.PQRId = hpqr.PQRId and e.Cierre = 'TRUE' FOR XML PATH('p')),1, 0, '')"
			,'tpqr.Nombre', "hpqr.Devoluciones"
			,'hpqr.FechaCierre'
			,"hpqr.Costos"
			,"(CASE WHEN hpqr.OtraCausa IS NOT NULL THEN hpqr.OtraCausa ELSE ca.Nombre End)"
			,"(CASE WHEN hpqr.OtraCalidad IS NOT NULL THEN hpqr.OtraCalidad ELSE q.Nombre End)"
			,"(CASE WHEN hpqr.OtraResponsable IS NOT NULL THEN hpqr.OtraResponsable ELSE r.Nombre End)"
			,"(CASE WHEN hpqr.OtraOperacion IS NOT NULL THEN hpqr.OtraOperacion ELSE o.Nombre End)"
			,"(CASE WHEN hpqr.OtraSeccion IS NOT NULL THEN hpqr.OtraSeccion ELSE s.Nombre End)"
			,'DATEPART(year, hpqr.Fecha)'
			,'DATEPART(month,  hpqr.Fecha)'
			,'DATEPART(day,  hpqr.Fecha)'
			,"STUFF((SELECT  cast(n.Fecha AS varchar) + ': ' + n.Detalle  +  '\r' FROM DetallePQR n LEFT JOIN EstadoPQR e ON e.EstadoId = n.EstadoReporte WHERE n.PQRId = hpqr.PQRId and n.Origen = 'c' FOR XML PATH('p')),1, 0, '')"
			,'hpqr.FechaCierre'
			,'epqr.Nombre'
			,"(CASE WHEN hpqr.ReclamoProveedor = 1 THEN 'SI' ELSE 'NO' End)"
			,"''" // La empresa
			,'DATEDIFF (DAY, hpqr.Fecha, hpqr.FechaCierre )'
			,'DATEPART(WEEk, hpqr.Fecha)'
		],
		filter_uColumn: false // Parámetro que se usa para el filtro de cada columna
	},
	processing: true,
	serverSide: true,
	order: [[0, 'ASC']],
	...
	buttons: [
		{ extend: 'colvis' },
		{ extend: 'copy', className: 'copyButton', exportOptions: {columns: ':not(:first-child)'}, title: 'Residente' },
		{ extend: 'csv', className: 'csvButton', exportOptions: {columns: ':not(:first-child)'}, title: 'Residente'},
		{ extend: 'excel', action: newExportAction, exportOptions: {columns: ':not(:first-child)'}, title: 'Residente'},
		{ extend: 'pdf', className: 'pdfButton', exportOptions: {columns: ':not(:first-child)'}, title: 'Residente'},
		{ extend: 'print', className: 'printButton', exportOptions: {columns: ':not(:first-child)', title: 'Residente'} }	
	],
	deferRender: true,
	scrollX: '100%',
...
$(function(){
	...
	if (!validarPedido){
		var inicio = $('[name=fecha1]').val();
		var fin = $('[name=fecha2]').val();
		where.push(
		...
		if ($('#nPqr').val().trim() != '') {
			where = [];
			where.push(['hpqr.PQR', $('#nPqr').val()]);
		} else if ($('#pedido').val().trim() != '') {
			where = [];
			where.push(['hpqr.Pedido', $('#pedido').val()]);
		}
		// Se toma como prioridad el número de la pqr
		if ($('#pedido').val().trim() != '' && $('#nPqr').val().trim() != '') {
			where = [];
			where.push(['hpqr.PQR', $('#nPqr').val()]);
		}
	} */
	...
	DTtblMatriz = dtSS(config);

	$("#btnCargar").click(function(e){
		e.preventDefault();
		var url = base_url()+"Administrativo/PQR/ConsultaPQR?\
		fecha1="+$('#fecha1').val()+'&\
		fecha2='+$('#fecha2').val()+'&\
	...
	$(document).on('focusin', '[data-foranea]', function(){
        lastFocus = $(this).val();
    });
    
    $(document).on("focusout", "#TerceroId", function(){
		if($(this).val().trim() != ''){
			cargarCliente($(this).val().trim(), this);
		} else {
			$("#TerceroId").val("");
			$("#ClienteNombre").text("Todos");
		}
	});
