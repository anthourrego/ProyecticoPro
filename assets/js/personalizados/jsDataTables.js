...
var domBftrip = "<'row no-gutters pt-1 px-1'<'col-sm-12 col-md-6 mb-2 mb-md-0'B><'col-sm-12 col-md-6'f>><'row'<'col-sm-12'tr>><'row'<'col-sm-12 col-md-5'i><'col-sm-12 col-md-7'p>>";
var domBftri = "<'row no-gutters pt-1 px-1'<'col-sm-12 col-md-6 mb-2 mb-md-0'B><'col-sm-12 col-md-6'f>><'row'<'col-sm-12'tr>><'row'<'col-12'i>>";
...
var newGraficarAction = function newGraficarAction(e, dt, button, config) {
	...
	for (var i = 0; i < config.columnas.data.length; i++) {
		optData += "<option selected value='"+config.columnas.data[i]+"'>"+dt.column(config.columnas.data[i]).header().innerHTML+"</option>";
	}
	for (var i = 0; i < config.columnas.value.length; i++) {
		optValue += "<option value='"+config.columnas.value[i]+"'>"+dt.column(config.columnas.value[i]).header().innerHTML+"</option>";
	}
	...
	alertify.cAlert().set({onshow:function(){
		...
		$('#cValor').change(function(){
			graficaValue = dt.columns([$(this).val()]).data().toArray();
			$('#cType').change();
		});
		...
		$('.bImg').click(function(e){
			...
			if(winPrint != null){
				winPrint.document.write("<a id='img' href='"+cAlertify.toDataURL('image/png')+"' download='grafica.png'><img src='"+cAlertify.toDataURL('image/png')+"'/></a>");
				winPrint.focus();
		...
		$('#cType').change(function(){
			...
			if(graficarInformacion.length > 1){
				for (var j = 0; j < graficarInformacion.length - 1; j++) {
					if(j == 0){
						newArray = graficarInformacion[j].map(function(e,i){return[e,graficarInformacion[j+1][i]]});
					}else{
						newArray = newArray.map(function(e,i){return e.concat(graficarInformacion[j+1][i])});
					}
				}
			}else{
					...
					tooltips: {
						callbacks: {
							label: function(tooltipItem, data){
								...
								return data.datasets[0].label + ': ' + Number(valor).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',');
							},
	...
	alertify.cAlert().set({onclose:function(){
		chart.destroy();
	...
	alertify.cAlert("<div class='col-sm-12' style='margin-top:5px;'> \
		<div class='row'> \
			<div class='col-sm-6'> \
				<div class='row'> \
					<div class='col-sm-6'> \
						<label style='float:right;'>Información&nbsp;:</label> \
						<button class='btn-sm btn btn-primary bImg' style='width:100%;'><i class='fas fa-save'></i> Guardar Imágen</button> \
					</div> \
					<div class='col-sm-6'> \
						<select multiple class='form-control input-sm form-group' id='cInfo' style='width:100%;display:block;'>"+optData+"</select> \
					</div> \
				</div> \
			</div> \
			<div class='col-sm-6'> \
				<div class='row'> \
					...
					<div class='col-sm-9'> \
						<select class='form-control input-sm form-group' id='cValor' style='width:100%;display:block;'>"+optValue+"</select> \
					</div> \
					<div class='col-sm-3'> \
...
alertify.cAlert || alertify.dialog('cAlert',function factory(){
	return {
