<div data-auto class="d-none">
	<label class="mb-0" for="EstadoIncidenciaId"><span class="text-danger">*</span>EstadoIncidenciaId</label>
	<input type="text" class="form-control form-control-sm data-int" data-db="EstadoIncidenciaId" data-required data-codigo data-identity-insert>
</div>
...
<label class="mb-0" for="CierreApp">CierreApp</label>
<select class="custom-select custom-select-sm" data-db="CierreApp">
	<option value="0">No</option>
	<option value="1">Sí</option>
</select>
...
<script type="text/javascript">
	...
	$PARAMETROS.createdRow = function(row, data, dataIndex){
		...
		$(row).find('td:eq(5)').css('background-color', color);
		$(row).find('td:eq(5)').css('color', lightOrDark(color));
