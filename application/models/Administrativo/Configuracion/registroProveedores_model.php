...
class RegistroProveedores_model extends CI_Model {
	...
	function cargar(){
		...
		SELECT
			...
			,RTRIM(PRO.Dias) Dias
			,RTRIM(PRO.ActividadId) ActividadId
			,RTRIM(A.nombre) AS ActividadIdNombre
			,RTRIM(PRO.TarifICA) TarifICA
			,RTRIM(PRO.TarifRetIv) TarifRetIv
			,RTRIM(PRO.TariReteId) TariReteId
			,RTRIM(TAR.tarifa) AS TariReteIdNombre
			,RTRIM(PRO.SobreBase) SobreBase
			,RTRIM(PRO.TipoTercId) TipoTercId
			,RTRIM(TT.nombre) AS TipoTercIdNombre
		FROM Tercero T
		LEFT JOIN Proveedor PRO ON T.TerceroID = PRO.TerceroId
		LEFT JOIN tipodocu TD ON T.tipodocuid = TD.tipodocuid
		LEFT JOIN TipoTerc TT ON PRO.TipoTercId = TT.tipotercid
		...
		LEFT JOIN Actividad A ON PRO.ActividadId = A.actividadid
		LEFT JOIN TariRete TAR ON PRO.TariReteId = TAR.tarireteid
	...
	function actualizar($cliente, $nombre, $value, $tabla) {
		...
		if($nombre == 'EsCliente'){
			if(count($this->db->query("SELECT TerceroID FROM Cliente WHERE TerceroID = '$cliente'")->result()) == 0){
		...
		$this->db->query("UPDATE Proveedor SET fechamodif = GETDATE() WHERE TerceroID = '$cliente'");
		if($this->db->trans_status() === FALSE){
			$this->db->trans_rollback();
			return 0;
		}else{
	...
	function qDescuentoFinancieroTercero($id = null){
		$cliente = $this->input->post('cliente');
		$sql = "SELECT
			DescuFinaTercId
			,TerceroId
			...
			,Porcentaje
			,Tipo 
		FROM DescuentoFinancieroTercero
		WHERE terceroid = '".$cliente."'
			AND Tipo = 'PR'";
		if($id != null){
			$sql.=" AND DescuFinaTercId = '$id'";
		}
		$consulta = $this->db->query($sql);
		return $consulta->result();
	}

	function qTarifaICATercero($id = null){
		$cliente = $this->input->post('cliente');
		$sql = "SELECT
			ICT.Id
			,TI.TarifICAId
		...
		FROM TarifaICATercero ICT
			LEFT JOIN TarifaICA TI ON ICT.TarifICAId = TI.TarifICAId
			LEFT JOIN Ciudad C ON TI.CiudadId = C.ciudadid
		WHERE ICT.terceroid = '".$cliente."'
			AND ICT.Tipo = 'PR'";
		if($id != null){
			$sql.=" AND DescuFinaTercId = '$id'";
		}
		...
		if(($btn == 'btnFastBackward' || $btn == 'btnFastForward') || ($btn == 'btnBackward' && $TerceroID == '') || ($btn == 'btnForward' && $TerceroID == '')){
			switch($btn){
				case 'btnFastBackward':
					$cliente = $this->db->query('SELECT TOP 1 TerceroID FROM Tercero WHERE EsProveedor = 1 ORDER BY TerceroID ASC');
				break;
				case 'btnFastForward':
					$cliente = $this->db->query('SELECT TOP 1 TerceroID FROM Tercero WHERE EsProveedor = 1 ORDER BY TerceroID DESC');
				break;
				case 'btnBackward':
					$cliente = $this->db->query('SELECT T.TerceroID AS TerceroID FROM
							(SELECT ROW_NUMBER() OVER(ORDER BY TerceroID ASC) AS n, TerceroID FROM Tercero WHERE EsProveedor = 1) T
						WHERE T.n = 1');
				break;
				case 'btnForward':
					$cliente = $this->db->query('SELECT T.TerceroID AS TerceroID FROM
		...
		if($codigo != 0){
			...
			try{
				...
				switch ($tabla) {
					...
					$array['nombre'] = '';
					break;
					case 'DescuentoFinancieroTercero':
					$array['Tipo'] = 'PR';
					break;
				}
				$this->db->insert($tabla, $array);
				...
				$CI->db->trans_begin();
					switch ($modulo) {
						case 'Clientes':
							...
							$_POST['RASTREO']['cambio'] = 'Modifica Proveedor '.$codigo.' Agrega Adjunto '.$nombreDoc;
	...
	function guardarICA(){
		...
		$TarifICAId = json_decode($this->input->post('TarifICAId'));
		$cliente = $this->input->post('cliente');
		$this->db->trans_begin();

		foreach ($TarifICAId as $key) {
			$Tarifa = $this->db->query("SELECT CiudadId, CASE WHEN Porcentaje IS NULL THEN 0.0000 ELSE Porcentaje END Porcentaje FROM TarifaICA WHERE TarifICAId = '$key'")->row();
			$_POST['RASTREO']['cambio'] = "Modifica Tercero ".$cliente." Agrega Tarifa ICA, ID ".$key." Ciudad ".$Tarifa->CiudadId." Tarifa ".strval($Tarifa->Porcentaje)." Tipo PR";
			try{
				$this->db->query("INSERT INTO TarifaICATercero (TerceroId, TarifICAId, Tipo) VALUES ('$cliente', '$key', 'PR')");
			}catch(Exception $e){
				$this->db->trans_rollback();
				return 0;
			}
	...
	function quitarICA(){
		...
		$this->db->trans_begin();
		try{
			$this->db->query("DELETE TarifaICATercero WHERE TerceroId = '$cliente'");
		}catch(Exception $e){
			$this->db->trans_rollback();
			return 0;
		}
