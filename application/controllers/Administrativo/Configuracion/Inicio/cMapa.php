<?php 

defined('BASEPATH') OR exit('No direct script access allowed');

class cMapa extends MY_CRUD_Controller {

	function __construct() {
		parent::__construct();

		if (!$this->session->userdata("login") || !in_array(1704, $this->session->userdata('SEGUR'))){
			redirect(base_url());
		}

		$this->load->model(array('Administrativo/Configuracion/Inicio/mMapa'));
	}

	function index(){
		$contenido['content_page'] = 'Administrativo/Configuracion/Inicio/vMapa';
		$contenido['titulo'] = 'Mapa de zonas';
		$contenido['js_lib'] = array(
			'dataTables/jquery.dataTables.min.js'
			,'dataTables/dataTables.bootstrap4.min.js'
			
			,'dataTables/dataTables.scroller.min.js'
			,'dataTables/responsive.bootstrap.min.js'
			,'dataTables/dataTables.buttons.min.js'
			,'dataTables/buttons.flash.min.js'
			,'dataTables/jszip.min.js'
			,'dataTables/pdfmake.min.js'
			,'dataTables/vfs_fonts.js'
			,'dataTables/buttons.html5.min.js'
			,'dataTables/buttons.print.min.js'
			,'personalizados/jsDataTables.js'
			,'chosen/chosen.jquery.min.js'
			,'inputmask/jquery.inputmask.bundle.min.js'
			,'datetimepicker/moment.js'
			,'datetimepicker/bootstrap-datetimepicker.min.js'
			,'personalizados/datepicker.js'
		);
		$contenido['css_lib'] = array(
			'datetimepicker/bootstrap-datetimepicker.min.css'

			,'dataTables/datatables.min.css'
			,'dataTables/dataTables.bootstrap4.min.css'
			,'dataTables/buttons.dataTables.min.css'

			,'chosen/chosen.min.css'
		);
		$contenido['script_adicional'] = array(
			'personalizados/Administrativo/Configuracion/Inicio/jsMapa.js'
		);

		/* $contenido['hBack'] 	= 'Administrativo/Configuracion/Menu';
		$contenido['txtBack'] 	= 'Configuración'; */

		$contenido["breadcrumb"] = [
			["nombre" => "Configuración", "ruta" => "Administrativo/Configuracion/Menu/Inicio/Configuracion"]
			,["nombre" => $contenido['titulo'], "ruta" => $this->uri->uri_string]
		];

		$this->load->view('UI/gestion_view', $contenido);
	}

	function obtenerVivienda(){
		if ($this->input->is_ajax_request()) {
			echo json_encode($this->mMapa->obtenerVivienda());
		}else{
			show_404();
		}
	}

	function obtenerDataVivienda(){
		if ($this->input->is_ajax_request()) {
			echo json_encode($this->mMapa->obtenerDataVivienda());
		}else{
			show_404();
		}
	}

	function guardarZona(){
		if ($this->input->is_ajax_request()) {
			echo json_encode($this->mMapa->guardarZona());
		}else{
			show_404();
		}
	}

	function obtenerZonas(){
		if ($this->input->is_ajax_request()) {
			echo json_encode($this->mMapa->obtenerZonas());
		}else{
			show_404();
		}
	}

	function actualizarImagen(){
		if ($this->input->is_ajax_request()) {
			echo json_encode($this->mMapa->actualizarImagen());
		}else{
			show_404();
		}
	}

	function guardarPosicionMapa(){
		if ($this->input->is_ajax_request()) {
			echo json_encode($this->mMapa->guardarPosicionMapa());
		}else{
			show_404();
		}
	}

	function modalData(){
		$contenido['TipoVivienda'] 	= $this->db->query("SELECT TipoViviendaId As id,nombre FROM TipoVivienda WHERE Estado = 'A'")->result();
		$this->load->view('Administrativo/Configuracion/Inicio/vModalMapa',$contenido);
	}

	function obtenerDataZona(){
		if ($this->input->is_ajax_request()) {
			echo json_encode($this->mMapa->obtenerDataZona());
		}else{
			show_404();
		}
	}

	function guardarConfiguracion(){
		if ($this->input->is_ajax_request()) {
			echo json_encode($this->mMapa->guardarConfiguracion());
		}else{
			show_404();
		}
	}

	function obtenerDataSeleccion(){
		if ($this->input->is_ajax_request()) {
			echo json_encode($this->mMapa->obtenerDataSeleccion());
		}else{
			show_404();
		}
	}

	function eliminarZona(){
		if ($this->input->is_ajax_request()) {
			echo json_encode($this->mMapa->eliminarZona());
		}else{
			show_404();
		}
	}

	function eliminarVivienda(){
		if ($this->input->is_ajax_request()) {
			echo json_encode($this->mMapa->eliminarVivienda());
		}else{
			show_404();
		}
	}
}
?>